...
void Player::createPlayer(std::string playerType, int colourCode, std::string AIType = "none", int size = -1, bool isParallelisedParam = false)
{
    if (playerType == "human")
    {
        isComputer = false;
...
void Player::playMoveAI(std::vector<std::pair<int, int>> availableTiles, int boardSize, int *x, int *y, std::vector<std::vector<int>> boardMatrix)
{
    ...
    else if (AIModel == "minimax")
    {
        ...
        auto minimax_pair = minimaxEngine.go(boardMatrix, blacksTurn);
